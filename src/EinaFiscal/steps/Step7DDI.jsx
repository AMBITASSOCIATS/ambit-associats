// steps/Step7DDI.jsx — Pas 7: Deducció per doble imposició internacional (Art. 48)
...
const TIPUS_RENDA_EXT = [
  ...
  { value: 'PLUSVALUES', label: 'Plusvàlues' },
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          ...
          <input
            ...
            placeholder="Ex: Dividends Empresa Espanyola, Interessos Banc Francès..."
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#009B9C]/40"
          />
        </div>
        ...
        <InputNum label="Import brut (€)" value={renda.importBrut} onChange={v => {
          update('importBrut', v);
          update('importNet', v - renda.retencioOrigen);
        }} />
        <InputNum label="Retenció practicada al país d'origen (€)" value={renda.retencioOrigen} onChange={v => {
        ...
        <div className="mt-3 bg-gray-50 rounded-lg p-3 text-xs">
          <div className="grid grid-cols-3 gap-2">
            <div>
              ...
              <p className="font-bold text-gray-800">{ddiInfo.quotaAndorra.toFixed(2)} €</p>
            </div>
            <div>
              <span className="text-gray-500">Retenció origen:</span>
              <p className="font-bold text-gray-800">{renda.retencioOrigen.toFixed(2)} €</p>
            </div>
            <div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="font-bold text-gray-800">Deducció per doble imposició (DDI)</h2>
              <p className="text-xs text-gray-500">Art. 48 Llei 5/2014 · Convenis de doble imposició</p>
            </div>
          </div>
          <button
            onClick={addRenda}
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-700 mb-4">
          <strong>Art. 48 Llei 5/2014:</strong> La DDI = mínim entre (a) l'impost efectivament pagat a l'estranger i (b) la quota andorrana que correspondria a aquella renda. Si hi ha CDI, la retenció al país d'origen es limita al tipus màxim convencional.
        </div>

        {dades.rendesExterior.length === 0 && (
          <div className="text-center py-8 text-gray-400">
            <p className="text-sm">Cap renda exterior afegida.</p>
            <p className="text-xs mt-1">Afegiu les rendes obtingudes a l'estranger amb retenció al país d'origen.</p>
          </div>
        )}

        {dades.rendesExterior.length > 0 && (
          <div className="bg-[#009B9C]/5 border border-[#009B9C]/20 rounded-xl p-3 text-sm">
            <span className="font-semibold text-[#009B9C]">Total DDI deduïble: </span>
